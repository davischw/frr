 ...
 * Callers  : (TBD)
 *---------------------------------------------------------------------------*/
void rsvp_packet_output_forward(struct stream *s, size_t size)
{
	//s->putp += size;
 ...
 * Callers  : (TBD).
 *------------------------------------------------------------------------*/
void rsvp_packet_free(struct rsvp_packet *rsvp_packet)
{
...
struct rsvp_packet_fifo *rsvp_packet_fifo_new(void)
{
	...
	rsvp_packet_fifo = XCALLOC(MTYPE_RSVP_PACKET_FIFO, sizeof(struct rsvp_packet_fifo));
 ...
 * Callers  : (TBD).
 *--------------------------------------------------------------------------*/
void rsvp_packet_fifo_push(struct rsvp_packet_fifo *rsvp_packet_fifo,
			   struct rsvp_packet *rsvp_packet)
{
 ...
 * Callers  : (TBD).
 *--------------------------------------------------------------------------*/
struct rsvp_packet *rsvp_packet_fifo_pop(struct rsvp_packet_fifo *rsvp_packet_fifo)
{
	struct rsvp_packet *rsvp_packet;
 ...
 * Callers  : (TBD).
 *---------------------------------------------------------------------------*/
struct rsvp_packet *rsvp_packet_fifo_head(struct rsvp_packet_fifo *rsvp_packet_fifo)
{
 ...
 * Callers  : TBD.
 *--------------------------------------------------------------------------*/
void rsvp_packet_fifo_flush(struct rsvp_packet_fifo *rsvp_packet_fifo)
{
	struct rsvp_packet *rsvp_packet;
 ...
 * Callers  : TBD
 *--------------------------------------------------------------------------*/
void rsvp_packet_fifo_free(struct rsvp_packet_fifo *rsvp_packet_fifo)
{
 ...
 * Callers  : (TBD)
 *---------------------------------------------------------------------------*/
void rsvp_packet_add_to_if(struct rsvp_if *rsvp_if, struct rsvp_packet *rsvp_packet)
{
 ...
 * Callers  : (TBD)
 *--------------------------------------------------------------------------*/
void rsvp_packet_del_from_if(struct rsvp_if *rsvp_if)
{
	struct rsvp_packet *rsvp_packet;
 ...
 * Callers  : (TBD)
 *-------------------------------------------------------------------------*/
struct stream *rsvp_stream_copy(struct stream *new, struct stream *s)
{
	new->endp = s->endp;
	//new->putp = s->putp;
	new->getp = s->getp;
 ...
 * Callers  : 
 *--------------------------------------------------------------------------*/
void rsvp_packet_decode_msg_hdr(struct stream *s, struct rsvp_msg_hdr *hdr)
{
	u_char byte;
	/* Sanity Check. */
	if (!s || !hdr)
 ...
 *-------------------------------------------------------------------------*/
int rsvp_packet_verify_checksum(struct rsvp_msg_hdr *hdr)
{
	u_int32_t ret;
	u_int32_t sum;
 ...
 * Callers  : rsvp_read.
 *--------------------------------------------------------------------------*/
int rsvp_packet_verify_msg_hdr(struct rsvp_packet *rsvp_packet, struct rsvp_if *rsvp_if,
 ...
 *-------------------------------------------------------------------------*/
void rsvp_packet_path_receive(struct rsvp_msg_hdr *rsvp_msg_hdr, struct rsvp_packet *rsvp_packet,
			      struct rsvp_if *rsvp_if)
{
	...
	while (size) {
		...
		case RSVP_CLASS_HOP:
			read = rsvp_obj_decode_hop(s, obj_ctype, obj_len, &path_msg_info.hop_info);
		...
		case RSVP_CLASS_LRO:
			read = rsvp_obj_decode_lro(s, obj_ctype, obj_len, &path_msg_info.lro_info);
			SET_FLAG(path_msg_info.cflags, PATH_MSG_CFLAG_LRO);
			size -= obj_len;
			break;

		case RSVP_CLASS_SA:
			read = rsvp_obj_decode_sa(s, obj_ctype, obj_len, &path_msg_info.sa_info);
		...
		case RSVP_CLASS_ST:
			read = rsvp_obj_decode_st(s, obj_ctype, obj_len, &path_msg_info.st_info);
 ...
 * Callers  : rsvp_read in rsvp_packet.c
 *-------------------------------------------------------------------------*/
void rsvp_packet_resv_receive(struct rsvp_msg_hdr *rsvp_msg_hdr, struct rsvp_packet *rsvp_packet,
			      struct rsvp_if *rsvp_if)
{
	...
	while (size) {
		...
		case RSVP_CLASS_HOP:
			read = rsvp_obj_decode_hop(s, obj_ctype, obj_len, &resv_msg_info.hop_info);
 ...
 * Callers  : rsvp_read in rsvp_packet.c
 *-------------------------------------------------------------------------*/
void rsvp_packet_path_err_receive(struct rsvp_msg_hdr *rsvp_msg_hdr,
				  struct rsvp_packet *rsvp_packet, struct rsvp_if *rsvp_if)
{
	return;
}
 ...
 * Callers  : rsvp_read in rsvp_packet.c
 *-------------------------------------------------------------------------*/
void rsvp_packet_resv_err_receive(struct rsvp_msg_hdr *rsvp_msg_hdr,
				  struct rsvp_packet *rsvp_packet, struct rsvp_if *rsvp_if)
{
	return;
}
 ...
 * Callers  : rsvp_read in rsvp_packet.c
 *-------------------------------------------------------------------------*/
void rsvp_packet_path_tear_receive(struct rsvp_msg_hdr *rsvp_msg_hdr,
				   struct rsvp_packet *rsvp_packet, struct rsvp_if *rsvp_if)
{
	return;
}
 ...
 * Callers  : rsvp_read in rsvp_packet.c
 *-------------------------------------------------------------------------*/
void rsvp_packet_resv_tear_receive(struct rsvp_msg_hdr *rsvp_msg_hdr,
				   struct rsvp_packet *rsvp_packet, struct rsvp_if *rsvp_if)
{
	return;
}
 ...
 * Callers  : rsvp_read in rsvp_packet.c
 *-------------------------------------------------------------------------*/
void rsvp_packet_resv_conf_receive(struct rsvp_msg_hdr *rsvp_msg_hdr,
				   struct rsvp_packet *rsvp_packet, struct rsvp_if *rsvp_if)
{
	return;
}
 ...
 * Callers  : rsvp_read in rsvp_packet.c
 *-------------------------------------------------------------------------*/
void rsvp_packet_hello_receive(struct rsvp_msg_hdr *rsvp_msg_hdr, struct rsvp_packet *rsvp_packet,
			       struct rsvp_if *rsvp_if)
{
	return;
}
 ...
 * Callers  : rsvp_read in rsvp_packet.c
 *-------------------------------------------------------------------------*/
void rsvp_packet_bundle_receive(struct rsvp_msg_hdr *rsvp_msg_hdr, struct rsvp_packet *rsvp_packet,
				struct rsvp_if *rsvp_if)
{
	return;
}
...
void rsvp_packet_send_path(struct rsvp_ip_hdr_info *ip_hdr_info,
			   struct rsvp_path_msg_info *path_msg_info, struct in_addr *nhop,
...
void rsvp_packet_send_resv(struct rsvp_ip_hdr_info *ip_hdr_info,
			   struct rsvp_resv_msg_info *resv_msg_info, struct in_addr *nhop,
			   struct rsvp_if *rsvp_if_out)
{
	return;
}
...
void rsvp_packet_send_path_err(struct rsvp_ip_hdr_info *ip_hdr_info,
			       struct rsvp_path_err_msg_info *path_err_msg_info,
			       struct in_addr *nhop, struct rsvp_if *rsvp_if_out)
{
	return;
}
...
void rsvp_packet_send_resv_err(struct rsvp_ip_hdr_info *ip_hdr_info,
 ...
 * Callers  : (TBD).
 *--------------------------------------------------------------------------*/
void rsvp_packet_send_path_tear(struct rsvp_ip_hdr_info *ip_hdr_info,
 ...
 * Callers  : (TBD).
 *--------------------------------------------------------------------------*/
void rsvp_packet_send_resv_tear(struct rsvp_ip_hdr_info *ip_hdr_info,
 ...
 * Callers  : (TBD).
 *--------------------------------------------------------------------------*/
void rsvp_packet_send_resv_conf(struct rsvp_ip_hdr_info *ip_hdr_info,
 ...
 * Callers  : (TBD).
 *--------------------------------------------------------------------------*/
void rsvp_packet_send_hello_req(struct rsvp_ip_hdr_info *ip_hdr_info,
				struct rsvp_hello_msg_info *hello_msg_info, struct in_addr *nhop,
				struct rsvp_if *rsvp_if_out)
{
	return;
}

void rsvp_packet_send_hello_ack(struct rsvp_ip_hdr_info *ip_hdr_info,
				struct rsvp_hello_msg_info *hello_msg_info, struct in_addr *nhop,
				struct rsvp_if *rsvp_if_out)
{
	return;
 ...
 *            rsvp_packet.c
 *--------------------------------------------------------------------------*/
struct rsvp_packet *rsvp_packet_receive(int sock, struct interface **ifp)
{
	int ret;
	struct ip iph;
	u_int16_t ip_len;
	...
	struct msghdr msgh = { NULL, 0, &iov, 1, buff, sizeof(*cmsg) + sizeof(*pktinfo), 0 };
 ...
 *---------------------------------------------------------------------------*/
int rsvp_write(struct thread *thread)
{
 ...
 *---------------------------------------------------------------------------*/
int rsvp_read(struct thread *thread)
{
	int ret;
	struct rsvp_packet *rsvp_packet;
	struct rsvp *rsvp;
