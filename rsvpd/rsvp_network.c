...
int rsvp_sock_init(void)
{
    ...
    *-----------------------------------------------------------------------*/
	...
	ret = setsockopt(rsvp_sock, IPPROTO_IP, IP_TOS, (char *)&tos, sizeof(int));
    ...
    *-----------------------------------------------------------------------*/
	ret = setsockopt(rsvp_sock, IPPROTO_IP, IP_HDRINCL, &hincl, sizeof(hincl));
	if (ret < 0)
    ...
    * the option set.
    *----------------------------------------------------------------------*/
	ret = setsockopt(rsvp_sock, IPPROTO_IP, IP_ROUTER_ALERT, &ralert, sizeof(ralert));
	/*-------------------------------------------------------------------------
