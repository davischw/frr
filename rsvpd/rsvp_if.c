 ...
 *--------------------------------------------------------------------------*/
int rsvp_if_event_handle(rsvp_if_event event, struct interface *ifp)
{
 ...
 *---------------------------------------------------------------------------*/
static int rsvp_if_up(struct interface *ifp)
{
 ...
 * Callers  : rsvp_if_event_handle in rsvp_if.c
 *--------------------------------------------------------------------------*/
static int rsvp_if_down(struct interface *ifp)
{
	struct rsvp_if *rsvp_if;
	/* Sanity Check. */
	if (!ifp)
 ...
 * Callers  : rsvp_if_event_handle in rsvp_if.c
 *---------------------------------------------------------------------------*/
static int rsvp_if_ip_en(struct interface *ifp)
{
	struct rsvp_if *rsvp_if;
	/* Sanity Check. */
	if (!ifp)
 ...
 * Callers  : rsvp_if_event_handle in rsvp_if.c
 *--------------------------------------------------------------------------*/
static int rsvp_if_ip_dis(struct interface *ifp)
{
	struct rsvp_if *rsvp_if;
	/* Sanity Check. */
	if (!ifp)
 ...
 *--------------------------------------------------------------------------*/
static int rsvp_if_create(struct interface *ifp)
{
	struct rsvp_if *rsvp_if;
 ...
 *--------------------------------------------------------------------------*/
static int rsvp_if_destroy(struct interface *ifp)
{
	struct rsvp_if *rsvp_if;
 ...
 * Callers  : 
 *---------------------------------------------------------------------------*/
void rsvp_if_enable_rsvp(struct interface *ifp)
{
	struct rsvp_if *rsvp_if;
	/* Sanity Check. */
	if (!ifp)
 ...
 * Callers  :
 *----------------------------------------------------------------------------*/
void rsvp_if_disable_rsvp(struct interface *ifp)
{
	struct rsvp_if *rsvp_if;
	/* Sanity Check. */
	if (!ifp)
 ...
 * Callers  : (TBD)..
 *---------------------------------------------------------------------------*/
struct prefix *rsvp_if_get_addr(struct rsvp_if *rsvp_if)
{
 ...
 * Callers  : rsvp_read.
 *--------------------------------------------------------------------------*/
struct rsvp_if *rsvp_if_lookup_by_local_addr(struct in_addr addr)
{
 ...
 *---------------------------------------------------------------------------*/
void rsvp_if_flag_dump_vty(struct vty *vty, unsigned long flag)
{
	int separator = 0;
 ...
 *--------------------------------------------------------------------------*/
int rsvp_if_prefix_vty_out(struct vty *vty, struct prefix *p)
{
 ...
 *--------------------------------------------------------------------------*/
void rsvp_if_connected_dump_vty(struct vty *vty, struct connected *connected)
{
	struct prefix *p;
	struct interface *ifp;
 ...
 *--------------------------------------------------------------------------*/
void rsvp_if_nd_dump_vty(struct vty *vty, struct interface *ifp)
{
	struct zebra_if *zif;
 ...
 *--------------------------------------------------------------------------*/
void rsvp_if_dump_vty(struct vty *vty, struct interface *ifp)
{
	...
	vty_out(vty,
		...
		ifp->stats.rx_packets, ifp->stats.rx_bytes, ifp->stats.rx_dropped,
		ifp->stats.rx_multicast, VTY_NEWLINE);
	...
	vty_out(vty,
		...
		ifp->stats.rx_crc_errors, ifp->stats.rx_frame_errors, ifp->stats.rx_fifo_errors,
		ifp->stats.rx_missed_errors, VTY_NEWLINE);

	vty_out(vty, "    output packets %lu, bytes %lu, dropped %lu%s", ifp->stats.tx_packets,
		ifp->stats.tx_bytes, ifp->stats.tx_dropped, VTY_NEWLINE);
	...
	vty_out(vty,
		...
		ifp->stats.tx_window_errors, VTY_NEWLINE);
	...
	vty_out(vty,
		...
		ifp->stats.ifi_ipackets, ifp->stats.ifi_ibytes, ifp->stats.ifi_iqdrops,
		ifp->stats.ifi_imcasts, VTY_NEWLINE);

	vty_out(vty, "    input errors %qu%s", ifp->stats.ifi_ierrors, VTY_NEWLINE);

	vty_out(vty, "    output packets %qu, bytes %qu, multicast packets %qu%s",
		ifp->stats.ifi_opackets, ifp->stats.ifi_obytes, ifp->stats.ifi_omcasts, VTY_NEWLINE);

	vty_out(vty, "    output errors %qu%s", ifp->stats.ifi_oerrors, VTY_NEWLINE);

	vty_out(vty, "    collisions %qu%s", ifp->stats.ifi_collisions, VTY_NEWLINE);
#else
	/* Statistics print out using sysctl (). */
	vty_out(vty,
		"    input packets %lu, bytes %lu, dropped %lu,"
 ...
 *-------------------------------------------------------------------------*/
void rsvp_if_dump_private_info_vty(struct vty *vty, struct interface *ifp)
{
	struct rsvp_if *rsvp_if;
	...
	vty_out(vty, "  Number of sessions : %d%s", rsvp_if->session_num, VTY_NEWLINE);

	vty_out(vty, "  RSVP-TE Parameters %s", VTY_NEWLINE);
	vty_out(vty, "  ================== %s", VTY_NEWLINE);
	/* Display Refresh Interval. */
	vty_out(vty, "  Refresh Interval : %d s", rsvp_if->v_refresh_interval);
	vty_out(vty, CHECK_FLAG(rsvp_if->sflags, RSVP_IF_SFLAG_REF_INT) ? "%s" : " (Default)%s",
		VTY_NEWLINE);

	/* Display Refresh Multiple. */
	vty_out(vty, "  Refresh Multiple : %d  ", rsvp_if->v_refresh_multiple);
	vty_out(vty, CHECK_FLAG(rsvp_if->sflags, RSVP_IF_SFLAG_REF_MUL) ? "%s" : " (Default)%s",
		VTY_NEWLINE);

	/* Display Hello Interval. */
	vty_out(vty, "  Hello Interval   : %d s", rsvp_if->v_hello_interval);
	vty_out(vty, CHECK_FLAG(rsvp_if->sflags, RSVP_IF_SFLAG_HELLO_INT) ? "%s" : " (Default)%s",
		VTY_NEWLINE);

	/* Display Hello Persistance. */
	vty_out(vty, "  Hello Persist    : %d  ", rsvp_if->v_hello_persist);
	vty_out(vty, CHECK_FLAG(rsvp_if->sflags, RSVP_IF_SFLAG_HELLO_PER) ? "%s" : " (Default)%s",
		VTY_NEWLINE);

	/* Display Hello Tolerance. */
	vty_out(vty, "  Hello Tolerance  : %d  ", rsvp_if->v_hello_tolerance);
	vty_out(vty, CHECK_FLAG(rsvp_if->sflags, RSVP_IF_SFLAG_HELLO_TOL) ? "%s" : " (Default)%s",
		VTY_NEWLINE);
...
struct cmd_node rsvp_interface_node = { INTERFACE_NODE, "%s(config-if)# ", 1 };
 ...
 *--------------------------------------------------------------------------*/
int rsvp_if_config_write(struct vty *vty)
{
	struct listnode *node;
	struct interface *ifp;
	char buf[BUFSIZ];
...
void rsvp_if_init(void)
{
    ...
    *-----------------------------------------------*/
	...
	install_element(INTERFACE_NODE, &no_mpls_traffic_eng_refresh_interval_cmd);
	install_element(INTERFACE_NODE, &mpls_traffic_eng_refresh_multiple_cmd);
	install_element(INTERFACE_NODE, &no_mpls_traffic_eng_refresh_multiple_cmd);
	...
	install_element(INTERFACE_NODE, &no_mpls_traffic_eng_message_bundling_cmd);
	install_element(INTERFACE_NODE, &mpls_traffic_eng_refresh_reduction_cmd);
	install_element(INTERFACE_NODE, &no_mpls_traffic_eng_refresh_reduction_cmd);
	return;
}
