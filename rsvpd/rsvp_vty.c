...
const char *route_type_str(u_char type)
{
	switch (type) {
...
/* Return route type string for VTY output.  */
const char route_type_char(u_char type)
{
	switch (type) {
	case ZEBRA_ROUTE_SYSTEM:
		return 'S';
...
void vty_show_ip_route_detail(struct vty *vty, struct route_node *rn)
{
	return;
}
 ...
 *--------------------------------------------------------------------------*/
void vty_show_ip_route(struct vty *vty, struct route_node *rn)
{
	...
	if ((rsvp_route_info = (struct rsvp_route_info *)rn->info) == NULL)
		return;
	/* Prefix Information. */
	len = vty_out(vty, "%c%c%c %s%c%d", route_type_char(rsvp_route_info->type), '>', '*',
	...
	if (rsvp_route_info->ifindex)
		vty_out(vty, ", %s%s", ifindex2ifname(rsvp_route_info->ifindex), VTY_NEWLINE);
 ...
 *----------------------------------------------------------------------*/
void rsvp_statistics_dump_vty(struct vty *vty, struct rsvp_statistics *statistics)
{
	/* Sanity Check. */
	if (!vty || !statistics)
		return;
 ...
 *            path to vty.
 *-------------------------------------------------------------------------*/
void rsvp_ip_path_dump_vty(struct vty *vty, struct rsvp_ip_path *rsvp_ip_path)
{
	...
	for (node = listhead((struct list *)rsvp_ip_path->ip_hop_list), count = 0; node;
	     nextnode(node), count++) {
		struct rsvp_ip_hop *rsvp_ip_hop = (struct rsvp_ip_hop *)node;
		char hop_addr_str[INET6_ADDRSTRLEN];

		inet_ntop(rsvp_ip_hop->hop.family, &rsvp_ip_hop->hop.u.prefix, hop_addr_str,
			  sizeof(hop_addr_str));
 ...
 *--------------------------------------------------------------------------*/
void rsvp_tunnel_if_dump_vty(struct vty *vty, struct rsvp_tunnel_if *rsvp_tunnel_if)
{
	/* Sanity Check. */
 ...
 *-------------------------------------------------------------------------*/
int ip_config_write_explicit_path(struct vty *vty, struct rsvp_ip_path *rsvp_ip_path)
{
	int write = 0;
	struct listnode *node;
	...
	for (node = listhead(rsvp_ip_path->ip_hop_list); node; nextnode(node)) {
		...
		char *hop_type_str = (rsvp_ip_hop->type == RSVP_IP_HOP_STRICT) ? "strict" : "loose";

		/* Extract the next hop address. */
		inet_ntop(rsvp_ip_hop->hop.family, &rsvp_ip_hop->hop.u.prefix, hop_addr_str,
			  sizeof(hop_addr_str));
		vty_out(vty, "    next-address %s %s%s", hop_addr_str, hop_type_str, VTY_NEWLINE);
		write++;
	}
 ...
 *--------------------------------------------------------------------------*/
int rsvp_tunnel_if_config_write(struct vty *vty, struct rsvp_tunnel_if *rsvp_tunnel_if)
{
	int write = 0;
	...
	for (i = 0; i < 6; i++) {
		...
		struct rsvp_tunnel_if_path_option *path_option = rsvp_tunnel_if->path_option + i;
		/* If this path-option is not valid then go to next option.*/
		if (!path_option->valid_flag)
			continue;

		path_type = (path_option->type == RSVP_TUNNEL_IF_PATH_OPTION_EXPLICIT) ? "explicit"
										       : "dynamic";
		vty_out(vty, "   tunnel mpls traffic-eng path-option %d %s", (i + 1), path_type);
...
#define SHOW_ROUTE_V4_HEADER                                                                       \
	"Codes: K - kernel route, C - connected, S - static, R - RIP, O - OSPF,%s       B - BGP, > - selected route, * - FIB route%s%s"
       ...
       "Select the loopback interface\n")
{
	...
	for (node = (struct listnode *)listhead(rm->rsvp->iflist); node; nextnode(node)) {
		...
		if (if_is_loopback(ifp)) {
			...
			vty_out(vty, "%% No Address Configured on interface %s%s", ifp->name,
				VTY_NEWLINE);
			return CMD_WARNING;
	...
	vty_out(vty, "%% No loopback interface is found in RSVP-TE Daemon%s", VTY_NEWLINE);
	return CMD_WARNING;
}
       ...
       "In Alphanumeric\n")
{
	...
	if (!rsvp_ip_path) {
		vty_out(vty, "ip explicit-path %s doesn't exist%s", argv[0], VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if (rsvp_ip_path->ref_cnt) {
		vty_out(vty, "Can't delete ip explicit-path %s%s", argv[0], VTY_NEWLINE);
        ...
        "Path Name (Optional)\n")
{
	...
	for (node = listhead((struct list *)rm->rsvp->ip_path_list); node; nextnode(node)) {
       ...
       "Interface number\n")
{
	...
	if (!rsvp_tunnel_if) {
		vty_out(vty, "%%Interface Tunnel %d creation failed%s", tunnel_num, VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "The tunnel number\n")
{
	...
	if (rsvp_tunnel_if_delete(tunnel_num)) {
		vty_out(vty, "%%Tunnel Interface %d Not Found%s", tunnel_num, VTY_NEWLINE);
		return CMD_WARNING;
	}
	return CMD_SUCCESS;
}
       ...
       "IPv4/v6 address of destination of the tunnel\n")
{
	...
	if (argc == 0) {
		vty_out(vty, "%% Destination Address Value Not Specified%s", VTY_NEWLINE);
		vty_out(vty, "Syntax :e.g  tunnel destination 10.10.10.1%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if (!str2prefix(argv[0], &dst)) {
		vty_out(vty, "%%Command Execution Failed Due To Internal Error%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "IPv4/v6 address of destination of the tunnel\n")
{
	...
	if (argc == 0) {
		vty_out(vty, "%% Destination Address Value Not Specified%s", VTY_NEWLINE);
		vty_out(vty, "Syntax : e.g, no tunnel destination 10.10.10.1%s", VTY_NEWLINE);
		return CMD_WARNING;
	}

	/* Check if any destination is set already.If not nothing to negate.*/
	if (RSVP_TUNNEL_IF_IS_DST_SET(rsvp_tunnel_if)) {
		vty_out(vty, "%%No Destination Configured On This Tunnel%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if (memcmp(&dst, &rsvp_tunnel_if->destination, sizeof(struct prefix))) {
		vty_out(vty, "%%No destination address %s found on Tunnel %d%s", argv[0],
			rsvp_tunnel_if->num, VTY_NEWLINE);
		return CMD_WARNING;
	}
...

{
	...
	if (rsvp_tunnel_if_path_option_is_valid(rsvp_tunnel_if, path_option_num)) {
		vty_out(vty, "%%Path Option %d is already set on this Tunnel%s", path_option_num,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if (!strncmp(argv[1], "e", 1)) {
		...
		if (!(path = rsvp_ip_path_search(argv[3]))) {
			vty_out(vty, "%% Explicit Path %s not found%s", argv[2], VTY_NEWLINE);
			return CMD_WARNING;
		}
	...
	rsvp_tunnel_if_set_path_option(rsvp_tunnel_if, path_option_num, path_option_type,
				       standby_flag, path);
	return CMD_SUCCESS;
}
       ...
       "Option Number\n")
{
	...
	if (!rsvp_tunnel_if_path_option_is_valid(rsvp_tunnel_if, path_option_num)) {
		vty_out(vty, "%%Can't delete path option %d%s", path_option_num, VTY_NEWLINE);
 ...
 *----------------------------------*/
int rsvp_interface_tunnel_from_vty(struct vty *vty, int argc, char **argv)
{
	int ret;
       ...
       "Tunnel number")
{
	...
	if (!rsvp_tunnel_if) {
		vty_out(vty, "%%The Tunnel Interface %d Not Found%s", tunnel_num, VTY_NEWLINE);
		return CMD_WARNING;
	}
 ...
 *-------------------------------------------------------------------------*/
int rsvp_show_interface_tunnel_from_vty(struct vty *vty, int argc, char **argv)
{
	int ret;
	...
	if (argc != 2) {
		...
		vty_out(vty, "[Usage] show interface tunnel [tunnel number]%s", VTY_NEWLINE);
		return CMD_WARNING;
	}

	ret = show_interface_tunnel_cmd.func(&show_interface_tunnel_cmd, vty, argc, argv);
...
*/
struct cmd_node rsvp_node = { RSVP_NODE, "%s(config-rsvp)# ", 1 };

struct cmd_node ip_node = { IP_NODE, "%s(config)# ", 1 };
/*
...
*/
struct cmd_node interface_tunnel_node = { INTERFACE_NODE, "%s(config-if)# ", 1 };
 ...
 *--------------------------------------------------------------------------*/
int mpls_config_write(struct vty *vty)
{
	struct rsvp *rsvp = rm->rsvp;
	char addr[32];
...
int rsvp_config_write(struct vty *vty)
{
	return 0;
}
 ...
 *--------------------------------------------------------------------------*/
int ip_config_write(struct vty *vty)
{
	int write = 0;
 ...
 *-------------------------------------------------------------------------*/
int interface_tunnel_config_write(struct vty *vty)
{
...
void rsvp_vty_init()
{
   ...
   */
   ...
   install_element (CONFIG_NODE, &rsvp_interface_tunnel_cmd);
	...
	install_element(CONFIG_NODE, &tunnel_mode_mpls_traffic_eng_cmd);
	install_element(CONFIG_NODE, &no_tunnel_mode_mpls_traffic_eng_cmd);

	install_element(CONFIG_NODE, &tunnel_destination_cmd);
	install_element(CONFIG_NODE, &no_tunnel_destination_cmd);

	install_element(CONFIG_NODE, &tunnel_mpls_traffic_eng_cspf_cmd);
	install_element(CONFIG_NODE, &no_tunnel_mpls_traffic_eng_cspf_cmd);

	install_element(CONFIG_NODE, &tunnel_mpls_traffic_eng_path_option_cmd);
	install_element(CONFIG_NODE, &no_tunnel_mpls_traffic_eng_path_option_cmd);
